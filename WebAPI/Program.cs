...
namespace WebAPI
{
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            {
                string originalTextaes = "Here is some data to encrypt! (AES)";
                byte[] encryptedaes = AesHelper.EncryptStringToBytes_Aes(originalTextaes, myAes.Key, myAes.IV);
                // 假設你有一個textBox用於顯示加密後的數據
                var Textaes = Convert.ToBase64String(encryptedaes);
                Console.WriteLine("Encrypted data (AES): " + Textaes);
                // 假設你有從某處獲得加密後的數據和密鑰及IV
                byte[] encryptedDataaes = Convert.FromBase64String(Textaes);
                string decryptedTextaes = AesHelper.DecryptStringFromBytes_Aes(encryptedDataaes, myAes.Key, myAes.IV);
                // 假設你有一個textBox用於顯示解密後的數據
                Textaes = decryptedTextaes;
                Console.WriteLine("Decrypted data (AES): " + Textaes);
            }
            //AES
            ...
            //RSA
            // 要加密的数据
            string dataToEncryptrsa = "My Secret Data! (RSA)";
            // 生成RSA密钥对
            (string publicKeyrsa, string privateKeyrsa) = RSAHelper.GenerateKeys();
            // 使用公钥加密
            byte[] encryptedDatarsa = RSAHelper.Encrypt(dataToEncryptrsa, publicKeyrsa);
            // 输出加密后的数据
            Console.WriteLine("Encrypted data (RSA): " + Convert.ToBase64String(encryptedDatarsa));
            // 使用私钥解密
            string decryptedDatarsa = RSAHelper.Decrypt(encryptedDatarsa, privateKeyrsa);
            // 输出解密后的数据
            Console.WriteLine("Decrypted data (RSA): " + decryptedDatarsa);
            //RSA


            //3DES
            string original3des = "Here is some data to encrypt (3DES)!";
            // 生成密钥和IV
            string key = TripleDESHelper.GenerateKey();
            string iv = TripleDESHelper.GenerateIV();
            // 加密
            byte[] encrypted3des = TripleDESHelper.Encrypt(original3des, key, iv);
            Console.WriteLine("Encrypted data (3DES): " + Convert.ToBase64String(encrypted3des));
            // 解密
            string decrypted3des = TripleDESHelper.Decrypt(encrypted3des, key, iv);
            Console.WriteLine("Decrypted data (3DES): " + decrypted3des);
            //3DES
